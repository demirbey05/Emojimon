  ...
  function execute(bytes memory data) public returns (bytes memory) {
    ...
    for (uint32 y = 0; y < height; y++) {
      for (uint32 x = 0; x < width; x++) {
        if (map[y][x] == TerrainType.None) continue;
        terrain[y * width + x] = bytes1(uint8(map[y][x]));
        if (map[y][x] == TerrainType.Boulder) {
    ...
    mapConfigComponent.set(MapConfig({ width: width, height: height, terrain: terrain }));
  }
}
